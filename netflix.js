...
const upcoming_el = document.querySelector('.upcoming .movies-grid')

const discover_el = document.querySelector('.discover .movies-grid')
            ...
            <div class="content">
                <div class="left">
                    ...
                    <div class="single-info">
                        ...
                        <button class="heart-icon">LIST/REMOVE<button>
                    </div>
                </div>
                <div class="right">
                    <h1>${movie.title}</h1>
...
async function add_to_dom_trending () {
    ...
    trending_el.innerHTML = data.slice(0, 6).map(e => {
        return `
            <div class="card" data-id="${e.id}">
                <div class="img">
                    <img src="${image_path + e.poster_path}">
...
add_to_dom_upcoming()
async function add_to_dom_upcoming () {

    const data = await get_trending_movies()
    console.log(data);

    upcoming_el.innerHTML = data.slice(0, 6).map(e => {
        return `
            <div class="card" data-id="${e.id}">
                <div class="img">
                    <img src="${image_path + e.poster_path}">
...
add_to_dom_discover()
async function add_to_dom_discover () {

    const data = await get_discover_movies()
    console.log(data);

    discover_el.innerHTML = data.slice(0, 8).map(e => {
        return `
            <div class="card" data-id="${e.id}">
                <div class="img">
                    <img src="${image_path + e.poster_path}">
